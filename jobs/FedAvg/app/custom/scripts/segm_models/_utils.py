...
def configure_logging(logger, log_level: int, log_file=None):
    """Define basic logging coniguration
    ...
    if log_file:
        ...
        logger.setLevel(
            logging.DEBUG if log_level == 10 else logging.INFO
        )
    else:
        # set log level of logger itself, as permissive as the most permissive of its handlers
...
class Base:
    ...
    def __init__(self, config):
        ...
        self.NUM_CLASSES = (
            len(self.config["data"]["masks"]["labels"]) + 1
    ...
    def save_npy(
        self, npy_arr: np.array, dst_dir: Path, npy_id: Path
    ):
        """
...
class ImageProcessor(Base):
    ...
    Image processor
    """
    #If one dataset is used then datapath is not needed and config["img_folder"] should be used
    def __init__(self, config, data_path):
        super().__init__(config)

        #self.src_img_dir = Path(self.data_root, config["img_folder"])
        self.src_img_dir = data_path
    ...
    def process_images(
        self, root: str, src_img_dir: Path = None, overwrite=True
    ):
        """
        ...
        split_img_dir = Path(
            self.split_root, root, self.config["img_folder"]
        )
        if split_img_dir.is_dir():
            if overwrite is True:
                ...
                save = False
                print(
                    f"Loading images from previously saved at '{split_img_dir}'..."
                )
            saved_images = sorted(
                list(split_img_dir.glob("**/*.npy"))
            )
        else:
            save = False
            print(
                f"Loading images from '{src_img_dir}', not saving them..."
            )
            split_img_dir = src_img_dir
            saved_images = []

        # get IDs of images to be processed
        image_IDs = read_txt_file(
            Path(split_img_dir.parent, root + ".txt")
        )
        ...
        if (
            ...
            _logger.info(
                f"Preprocessing images according to '{self.processing}' method..."
            )
            ...
            _logger.info(
                f"Loading previously preprocessed images from '{split_img_dir}'..."
            )
    ...
    def _process_basic(self, img_path):
        ...
        if self.only_tir == True:
            preproc_img = np.stack(
                (tir_img, tir_img, tir_img), axis=2
            )
        else:
    ...
    def _process_retinex_unsharp(self, img_path):
        ...
        retinex_rgb = cv2.bioinspired_Retina.create(
            (rgb_img.shape[1], rgb_img.shape[0]), colorMode=True
        )
        ...
        rgb_retinaOut_gray = cv2.cvtColor(
            rgb_retinaOut, cv2.COLOR_BGR2GRAY
        )
        ...
        retinex_tir = cv2.bioinspired_Retina.create(
        ...
        if self.CHANNELS == 3:
            ...
            if self.only_tir == True:
                preproc_img = np.stack(
                    (tir_img, tir_retinaOut, tir_um), axis=2
                )
            else:
                preproc_img = np.stack(
            ...
            rgb_um = cv2.normalize(
                rgb_um, None, 0, 255, cv2.NORM_MINMAX
            )
            ...
            raise ValueError(
                f"Expected either images with 3 or 4 channels for preprocessing with "
    ...
    def _stack_array(self, rgb_array, tir_array):
        ...
        if self.CHANNELS == 3:
            ...
            stacked_img = np.stack(
                (rgb_gray_img, tir_array, tir_array), axis=2
            )
        elif self.CHANNELS == 4:
            stacked_img = np.dstack(
                (rgb_array, tir_array[:, :, np.newaxis])
            )
        else:
            raise ValueError(
                f"Expected either images with 3 or 4 channels for preprocessing with "
    ...
    def _remove_vignetting(image):
        ...
        bin_mean = [
            np.mean(image.flat[digitized == i])
...
class MaskProcessor(Base):
    ...
    Mask processor
    """
    #If one dataset is used then datapath is not needed and config["mask_folder"] should be used
    def __init__(self, config, datapath):
        super().__init__(config)

        #self.src_mask_dir = Path(
        #    self.data_root, config["mask_folder"]
        #)
    ...
    def load_masks(
        ...
        if split_mask_dir.is_dir():
            if overwrite is True:
                ...
                save = False
                print(
                    f"Loading masks from previously saved at '{split_mask_dir}'..."
                )
            saved_masks = sorted(
                list(split_mask_dir.glob("**/*.npy"))
            )
        else:
            save = False
            print(
                f"Loading masks from '{src_mask_dir}', not saving them..."
            )
            split_mask_dir = src_mask_dir
            saved_masks = []

        # get IDs of masks to be processed
        mask_IDs = read_txt_file(
            Path(split_mask_dir.parent, root + ".txt")
        )
        ...
        if (
            not saved_masks
            or len(mask_IDs) != len(saved_masks)
            or overwrite
        ):
            _logger.info(
...
class ModelLoader(Base):
    ...
    def __init__(self, config, model_dir=None):
        ...
        assert Path(self.model_dir).exists(), (
            f"Provided model directory '{self.model_dir}' "
            f"does not exist."
        )
        # load model
    ...
    def load_model_from_dir(self):
        ...
        try:
            model_path = list(Path(self.model_dir).glob("**/*.hdf5"))[
                0
            ]
        except IndexError:
            _logger.warning(
                f"Model directory '{self.model_dir}' does not contain a .hdf5 path."
            )
            sys.exit(-1)
        ...
        METRICS_DICT[self.config["train"]["loss"]["name"]] = eval(
            self.config["train"]["loss"]["function"]
        )
    ...
    def confirm_model_expected_input(self):
        ...
        """
        _, size_h, size_w, channels = self.model.layers[
            0
        ].input_shape[0]
...
if __name__ == "__main__":
    from jobs.tensorflow.app.custom.scripts.configuration import (
        init_temp_conf,
    )
