...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generating segmentation masks from json "
        "annotation files"
    )
    parser.add_argument(
        "-j",
...
def create_segm_mask_from_json(
    json_dir: Path, img_dir: Path, save_for_view: bool
):
    """
    ...
    for dataset_path in img_dir.glob("*"):
        ...
        if dataset_path.is_file():
            _logger.info(
                f"Directory '{img_dir}' contains .npy files instead of further image "
            ...
            json_path = next(
                json_dir.glob("*.json")
            )  # should be only one annotation file
            break_bool = True

        else:
            # Find annotation file to match current dataset, skip if it doesn't exist
            json_path = Path(
                json_dir, f"{dataset_path.stem}_json.json"
            )
            if not json_path.exists():
                ...
                if matches:
                    _logger.info(
                        f"Found annotations '{matches[0]}' to replace '{json_path.name}'"
                    )
                    json_path = Path(json_dir, matches[0])
                else:
                    _logger.warning(
                        f"ANNOTATION FILE '{json_path.name}' DOES NOT EXIST, skipping!"
                    )
                    continue

        dataset_labels_df = extract_anno_df(
            json_path=json_path, set_dir=dataset_path
        )
...
def extract_anno_df(
    json_path: Path, set_dir: Path, descriptor="thermal_objects"
):
    """
    ...
    for idx, anno_img in enumerate(anno_img_data.values()):
        ...
        if not npy_path.exists():
            _logger.warning(
                f"{set_dir.name} - Numpy image file missing, skipping: '{npy_path}'"
            )
            skipped += 1
            continue
        ...
        for region in regions:
            ...
            if (anno["name"] == "polygon") or (
                ...
                _logger.warning(
                    f"{set_dir.name} - {img_stem} - unsupported shape {anno['name']}!"
                )
                raise KeyError(
            ...
            df.loc[len(df)] = [
                str(
                    Path(*npy_path.parts[-2:])
                ),  # image identifier (i.e. MU_02/DJI_0_0001.npy)
...
def create_masks(anno_df, dst_dir, save_for_viewing=False):
    ...
    for unique_img_id in tqdm(unique_image_ids):
        ...
        _logger.debug(
            f"Mask for '{unique_img_id}' has labels: {np.unique(combined_mc_mask)}"
        )
        ...
        if save_for_viewing:
            ...
            colormap = ListedColormap(
                config["data"]["masks"]["custom_colors"]
            )
