...
class FedOpt(FedAvg):
    ...
    def update_model(self, global_model: FLModel, aggr_result: FLModel):
            ...
            if self.current_round>0:
                print('we are in round 2 and we are loading the weights')
                global_model_tf.load_weights(self.persistor._model_save_path)
            ...
            for  key, param in global_model.params.items():
                if w_idx >= num_trainable_weights:  # Ensure we don't go out of bounds
                    break
            ...
            for key in global_model.params:
                ...
                if key in model_diff_params:
                    ...
                    if aggr_result.params_type == ParamsType.FULL:
                       new_weights[key] = aggr_result.params[key]
                    else:    
