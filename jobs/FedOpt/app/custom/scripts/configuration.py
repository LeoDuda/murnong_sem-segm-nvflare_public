...
_default_config = {
    "data_root": "/home/project/hk-project-test-p0021801/uvecw/MA/murnong_sem-segm/dataset/",
    "anno_root": "/home/project/hk-project-test-p0021801/uvecw/MA/murnong_sem-segm/dataset",
    "split_root": "dataset",
    "img_folder": "images",
    "mask_folder": "masks",
    ...
    "data": {
        ...
        "loader": {
            "SIZE_H": 512,  # 512,  # alternatively: 256
    ...
    "eval": {
        "SKLEARN_METRICS": {
            ...
            "accuracy": {"func": "accuracy_score", "params": {}},
            "balanced_accuracy": {
...
def init_temp_conf(delete_existing: bool = True):
    """
    ...
    if temp_conf_path.is_file() and delete_existing is False:
        try:
            ...
            print(
                "Previously saved config is corrupted or empty. Deleting and recreating..."
            )
...
def write_conf(conf: dict, config_path: Path):
    ...
    if config_path.suffix not in [".json"]:
        raise TypeError(
            f"Only .json configs allowed, but config is of type {config_path.suffix}!"
        )
...
# if temp_conf_path.exists() and temp_conf_path.is_file():
#    try:
#        config = read_conf(temp_conf_path)
#    except json.JSONDecodeError:
