...
def main(
    ...
    assert (
        SRC_DIR.is_dir()
    ), f"Provided source directory {SRC_DIR} is not an existing folder!"
    ...
    DATASET_NAMES = [
        dn.name
        for dn in sorted(masks_dir.glob("*"))
    ...
    images_train, images_test = split_train_test(
        images=image_identifiers,
        annotations=annotations_per_img,
        test_size=test_size,
        random_state=random_state,
        check_anno_distrib=check_anno,
        check_class_distrib=check_class,
    ...
    if destination_dir:
        # update config file
        update_conf(
            conf=config, params={"split_root": destination_dir}
        )
...
def split_train_test(
    ...
    if check_anno_distrib:
        ...
        true_test_size = test_counts / sum(
            len(sublist) for sublist in annotations
        )
        if true_test_size < np.maximum(
            0, test_size - 0.05
        ) or true_test_size > np.minimum(1, test_size + 0.05):
            _logger.warning(
    ...
    if check_class_distrib:
       ...
       '''missing_ids_train = [
            ...
            if not any(
                cat_id in sublist for sublist in annotations_train
            )
        ]
        missing_ids_test = [
            cat_id
            for cat_id in CATEGORY_IDs
            if not any(
                cat_id in sublist for sublist in annotations_test
            )
        ]'''
    ...
    if check_set_distrib:
        ...
        missing_dataset_train = [
            ...
            if dataset
            not in [img_path.parent.name for img_path in images_train]
        ]
        missing_dataset_test = [
            dataset
            for dataset in DATASET_NAMES
            if dataset
            not in [img_path.parent.name for img_path in images_test]
        ]
    ...
    _logger.info(
        f"Suitable split found at random_state {random_state}."
    )
    _logger.info(
        overview(
            images,
...
def build_dirtree():
    ...
    _logger.info(
        f"Built the test/train directory tree at '{DST_DIR}'"
    )
