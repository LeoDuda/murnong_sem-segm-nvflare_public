...
cuda_data_dir = os.path.join(
    os.environ["CUDA_HOME"], "nvvm/libdevice"
)
...
from jobs.tensorflow.app.custom.scripts.segm_models.tf_model import (
    UNet,
)
...
def main(
    ...
    def train(
        ...
        #     base_model = sm.Unet(backbone_name=config['model']['backbone'], encoder_weights='imagenet',
        #           classes=NUM_CLASSES)
    ...
    while flare.is_running():
        ...
        train(
            ...
            model_path=Path(
                model_dir,
                config["model"]["type"] + config["model"]["suffix"],
            ),
...
def save_model_info(model, model_dir: Path):
    ...
    with open(summary_path, "w") as summary_file:
        sys.stdout = (
            summary_file  # Redirect print statements to the file
        )
...
# nvflare simulator -n 2 -t 1 ./jobs/tensorflow -w client_api_ddp_workspace
