...
def main(
    ...
    def train(input_model=None):
        ...
        #combined_mean_scores, _=evaluate(model,config,X_test,y_test)

       # print('the combined mean scores are:', combined_mean_scores)
        combined_mean_scores_global = {f"{key}_global": value for key, value in combined_mean_scores_global.items()}
        #print(f'the global metrics are {combined_mean_scores_global}')
      ...
      #  else:
           # mlflow_writer.log_metrics(
             #   metrics=combined_mean_scores, step=input_model.current_round+1
#            )
        mlflow_writer.log_metrics(
            metrics=combined_mean_scores_global , step=input_model.current_round
        )
...
def load_data(site):
    ...
    if site=="site-1":
        data_path = "/hkfs/home/haicore/scc/mp9809/dataset_MU/"
    else:
        data_path ="/hkfs/home/haicore/scc/mp9809/dataset_KA/"  
   # data_path ="/hkfs/home/project/hk-project-test-p0021801/uvecw/datasets/dataset_KA/"      
    start = time.time()
