...
if [ -n "$src_path" ]; then
    training_cmd="$training_cmd --src-dir $src_path"
fi
if [ -n "$split_path" ]; then
    training_cmd="$training_cmd --split-dir $split_path"
fi
...
if [ -n "$default_log" ]; then
    training_cmd="$training_cmd $default_log"
fi
if [ -n "$verbosity" ]; then
