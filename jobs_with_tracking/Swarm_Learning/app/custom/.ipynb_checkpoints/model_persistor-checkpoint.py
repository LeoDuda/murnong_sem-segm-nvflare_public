...
class TFModelPersistor(ModelPersistor):
    def __init__(
        self, model: tf.keras.Model, N: int, save_name="tf_model.ckpt"
    ):
        super().__init__()
    ...
    def save_model(
        ...
        print(f'the task data is { current_round}')
        #total_rounds=fl_ctx.get_prop(AppConstants.NUM_ROUNDS)
        #print(f'the total_rounds is {total_rounds}')

        if current_round==3 :
            print('saving the model on the mlflow server')
