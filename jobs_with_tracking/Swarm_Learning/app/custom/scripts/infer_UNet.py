...
def main(
    ...
    _logger.info(
        f"Predicting on image '{img_path.name}' with model '{config['model']['type']}'"
    )
    infer_mask = predict(model=model, proc_img=proc_img)
    ...
    try:
        ...
        match_mask_path = Path(
            data_root, config["mask_folder"], mask_name
        )
        if match_mask_path.is_file():
            _logger.info(
                f"Ground truth segmentation mask found for image '{img_path.name}' at '{match_mask_path}'"
            )
    ...
    if mask_path is not None:
        ...
        _logger.info(
            f"Using ground truth segmentation mask at '{mask_path}' for comparison."
        )
...
def predict(model, proc_img):
    ...
    prediction = model.predict(
        np.expand_dims(proc_img, axis=0), verbose=1
    )
    class_predictions = np.argmax(
        prediction, axis=-1
    )  # max across NUM_CLASSES channels -> (1, SIZE_H, SIZE_W)
    class_predictions = np.squeeze(
        class_predictions
    )  # remove unnecessary dim -> (SIZE_H, SIZE_W)
...
def plot(
    ...
    rgb_infer_mask = (colormap(infer_mask) * 255).astype(np.uint8)[
        :, :, 0:3
    ]
    ...
    fig = plt.figure(
        constrained_layout=True, figsize=(num_channels * 6, 18)
    )
    fig.suptitle(
        f"Inference Results - {Path(src_img_path.parent.name, src_img_path.name)}",
        fontsize=22,
    )
    ...
    for row, subfig in enumerate(subfigs):
        ...
        if row == 0:
            ...
            if true_mask is not None:
                axs = subfig.subplots(nrows=1, ncols=2)
                axs[0].imshow(
                    (colormap(true_mask) * 255).astype(np.uint8)[
                        :, :, 0:3
                    ]
                )
                axs[0].set_title(
                    "True mask based on annotations", fontsize=15
                )
